...
first_email = 'abcdefghijklmnopqrstuvwxyz'
middle_email = 'abcdefghijklmnopqrstuvwxyz'
last_email = 'abcdefghijklmnopqrstuvwxyz'
...
def random_email():
    result = ""
    result += random.choice(first_email)
    result += random.choice(middle_email)
    result += random.choice(last_email)
    return result + str(random.randint(1, 1000)) + '@ssafy.com'
...
for product in baseList:
    for option in optionList:
        if product['fin_prdt_cd'] == option['fin_prdt_cd']:
            item = '' + product['fin_prdt_nm'] + '_' + option['save_trm']
...
dict_keys = ['username', 'email', 'financial_products', 'campus', 'generation', 'salary', 'fi_money', 'fixed_cost', 'rent_help' ]
...
while i < N:
    re = random_email()
    if re in useremail_list:
        continue

    useremail_list.append(re)
    i += 1

# 저장 위치는 프로젝트 구조에 맞게 수정합니다.
save_dir = './yo_bank_backend/accounts/fixtures/user_data.json'
with open(save_dir, 'w', encoding="utf-8") as f:
    ...
    for i in range(N):
        ...
        file["fields"] = {
            ...
            'money_for_financial' : random.randrange(1, 4000000, 490000),
            'left_money_for_financial' : 1,
            'left_money_for_financial' : 0,
            'salary' : random.randrange(2000000, 4000000, 100000),
            'password': "dbgudals123@",
            'is_active': True,
            'is_staff': False,
            'is_superuser': False
        }
